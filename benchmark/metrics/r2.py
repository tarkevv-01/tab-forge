...
def r2_metric(synthetic_data, real_data, model: str = 'xgboost') -> float:
    """
    Вычисляет R² коэффициент детерминации между реальными и синтетическими данными.
    Обучает модель на синтетических данных и тестирует на реальных.
    ...
    Returns:
        float: Значение R² метрики
    """
    # Проверяем типы задач
    if real_data.info.task_type != 'regression':
        raise ValueError("R² метрика поддерживается только для regression задач")
